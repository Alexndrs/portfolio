export function ProjectCard({ name, description, startDate, endDate, tags, githubLink, liveLink }: {
    ...
    return <div className="flex w-full bg-transparent hover:bg-black/2 dark:hover:bg-white/2 p-4 rounded-xl transition-colors duration-200 flex-col md:flex-row">
        <div className="w-24 flex flex-shrink-0 text-xs">{startDate}-{endDate}</div>
            ...
            <div className="highlight flex items-center">
                ...
                {liveLink ? (
                    <div className="ml-4 bg-green-500/20 dark:bg-green-600/20 px-2 py-[2px] rounded-full flex items-center gap-1">
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="text-green-600 dark:text-green-400">
